...
const DOCUMENT_LABELS = {
  ...
  govId: 'Government ID',
  sin: 'SIN Document',
  incorp: 'Incorporation Document',
  deposit: 'Direct Deposit (Void Check)',
};

function applicantOnboardingLink(applicationId, step = 1) {
...
async function computeOnboardingMissingFields(applicationId) {
  const [application, onboarding, docs, adminRequests] = await Promise.all([
    ...
    prisma.onboardingState.findUnique({ where: { applicationId }, select: { enabled: true } }),
    prisma.document.findMany({
      where: { applicationId, status: { in: ['uploaded', 'expiring_soon'] } },
      select: { templateKey: true },
    }),
    prisma.adminDocumentRequest.findMany({
      where: { applicationId },
  ...
  const missing = [];
  if (!application) return { application: null, missingFields: missing };

  // Profile step fields (these are the exact stored keys on Application)
  if (!application.dateOfBirth) missing.push('Date of Birth');
  if (!String(application.gender || '').trim()) missing.push('Gender');
  if (!String(application.nationality || '').trim()) missing.push('Nationality');
  if (!String(application.profilePhotoUrl || '').trim()) missing.push('Profile Photo');

  const requiredKeys = [
    ...MANDATORY_DOCUMENT_KEYS,
    ...adminRequests.map((r) => `adminreq_${r.id}`),
  ];
  const have = new Set(docs.map((d) => d.templateKey));
  const missingDocKeys = requiredKeys.filter((k) => !have.has(k));
  for (const key of missingDocKeys) {
    if (key.startsWith('adminreq_')) {
      const id = key.slice('adminreq_'.length);
      const req = adminRequests.find((r) => r.id === id);
      missing.push(req?.name || key);
    } else {
      missing.push(DOCUMENT_LABELS[key] || key);
    }
  }

  return { application, onboarding, missingFields: missing };
}
...
export async function adminEnableOnboarding(applicationId) {
  ...
  // Email applicant to complete onboarding with an explicit missing-items list (derived from actual stored fields)
  const { application: appRow, missingFields } = await computeOnboardingMissingFields(applicationId);
  if (appRow?.email) {
    await sendEmail({
      to: appRow.email,
      ...onboardingMissingDetailsEmail({
        name: appRow.name,
        missingFields,
        profileCompletionLink: applicantOnboardingLink(applicationId, 1),
      }),
    });
  }
